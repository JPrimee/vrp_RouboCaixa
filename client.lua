...
--- TEM QUE CONFIGURAR AS COOORDS DOS LOCAIS QUE VOCE QUER Q ROUBE OS CAIXA / VALOR E ETC
local robbing = false
local bank = ""
...
local banks = {
	["Lindsay Circus"] = {   
		...
		nameofbank = "Posto de gasolina de Lindsay Circus LTD",
		lastrobbed = 0
	},
	["Prosperity St"] = {
	...
	["North Rockford Dr"] = {
		...
		reward = 100 + math.random(800,1500),
		nameofbank = "Posto de gasolina North Rockford Dr LTD",
		lastrobbed = 0
	},
...
AddEventHandler('es_bank:toofarlocal', function(robb)
	robbing = false
	TriggerEvent('chatMessage', 'Aviso: COLOCA NOME DO SV', {255, 0, 0}, "O roubo Foi Cancelado, Você Não Receberá Nada.")
	robbingName = ""
	secondsRemaining = 0
	incircle = false
end)
...
AddEventHandler('es_bank:playerdiedlocal', function(robb)
	robbing = false
	TriggerEvent('chatMessage', 'Aviso: COLOCA NOME DO SV', {255, 0, 0}, "O roubo foi Cancelado, Você Morreu!")
	robbingName = ""
	secondsRemaining = 0
	incircle = false
end)
...
AddEventHandler('es_bank:robberycomplete', function(reward)
	robbing = false
	TriggerEvent('chatMessage', 'Aviso: COLOCA NOME DO SV', {255, 0, 0}, "Roubo Com Sucesso!, Você Recebeu:^2" .. reward)
	bank = ""
	secondsRemaining = 0
	incircle = false
end)
...
Citizen.CreateThread(function()
	while true do
				...
				if not robbing then
					...
					if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 2)then
						if (incircle == false) then
							bank_DisplayHelpText("Pressione ~INPUT_CONTEXT~ Para Roubar ~b~" .. v.nameofbank .. "~w~ Cuidado a Policia ")
						end
						incircle = true
		...
		if robbing then
			...
			bank_drawTxt(0.66, 1.44, 1.0,1.0,0.4, "Roubo: ~r~" .. secondsRemaining .. "~w~ Segundos Restantes", 255, 255, 255, 255)

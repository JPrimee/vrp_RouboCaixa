...
local banks = {
	...
	["Caixa Eletronico Banco Central"] = {
		position = { ['x'] = 147.62094116211, ['y'] = -1035.7580566406, ['z'] = 29.34308052063 },
		reward = 100 + math.random(800,5000),
		nameofbank = "Caixa Eletronico Banco Central",
		lastrobbed = 0
	},	
	["El Rancho Blvd"] = {
...
AddEventHandler('es_bank:toofar', function(robb)
	if(robbers[source])then
		...
		robbers[source] = nil
		TriggerClientEvent('chatMessage', -1, 'Noticia Urgente', {255, 0, 0}, "O roubo foi cancelado em: ^2" .. banks[robb].nameofbank)
	end
end)
...
AddEventHandler('es_bank:rob', function(robb)
  ...
  if vRP.hasGroup({user_id,"cop"}) then
    vRPclient.notify(player,{"~r~Policiais não podem roubar loja Sai fora Dai [Prime Da Olho]"})
  else
    if #cops >= 0 then 
	  if banks[robb] then
		  ...
		  if (os.time() - bank.lastrobbed) < 600 and bank.lastrobbed ~= 0 then
			  TriggerClientEvent('chatMessage', player, 'COLOCA NOME DO SV', {255, 0, 0}, "Esta loja já foi atingida. Você terá que esperar outro: ^2" .. (1200 - (os.time() - bank.lastrobbed)) .. "^0 Segundos.")
			  return
		  end
		  TriggerClientEvent('chatMessage', -1, 'Noticia Urgente', {255, 0, 0}, "Roubo em andamento em ^2" .. bank.nameofbank)
		  TriggerClientEvent('chatMessage', player, 'Aviso: COLOCA NOME DO SV', {255, 0, 0}, "Você começou um assalto em: ^2" .. bank.nameofbank .. "^0, não fique muito longe desse ponto!")
		  ...
		  SetTimeout(180000, function()
			  if(robbers[savedSource])then
				  if(user_id)then
					  ...
					  TriggerClientEvent('chatMessage', -1, 'Noticia Urgente', {255, 0, 0}, "Assalto a mais: ^2" .. bank.nameofbank .. "^0!")
      ...
      vRPclient.notify(player,{"~r~Não há Policiais Suficientes! Volte Outra hora Para Conversar com Gerente!"})
    end
	end
end)
---<<<<### Primezinhoo ###>>>>---
